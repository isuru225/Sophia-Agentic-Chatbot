...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="execute_sql",
            description="""
Execute read-only SQL queries on the MySQL database.
...
- purch_amt (DECIMAL)
- ord_date (DATE)
- customer_id (FK → Customer.c_id)
- salesman_id (FK → Salesman.s_id)
...
Salesman-related:
- "salesman name", "names", "salesman names" →
  CONCAT(Salesman.FirstName, ' ', Salesman.LastName)

- "first name" → Salesman.FirstName
- "last name" → Salesman.LastName
- "salesman id" → Salesman.s_id
- "salesman city" → Salesman.City
- "commission" → Salesman.commission

Customer-related:
- "customer name" → Customer.cust_name
- "customer id" → Customer.c_id
- "customer city" → Customer.city
- "grade" → Customer.grade

Order-related:
- "order number" → Orders.ord_no
- "order date" → Orders.ord_date
- "order amount", "purchase amount" → Orders.purch_amt
- "total sales", "total amount" → SUM(Orders.purch_amt)

Relationships:
- Orders.salesman_id → Salesman.s_id
- Orders.customer_id → Customer.c_id
...
- Map user terms to most appropriate schema terms.
- Use JOINs when data spans multiple tables
- Never guess column or table names
- Generate valid MySQL SQL only
